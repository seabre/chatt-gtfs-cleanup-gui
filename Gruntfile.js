...
module.exports = function (grunt) {
  ...
  sass: { 
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/chattGtfsCleanupGui.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      app: {
        files: [{
          ...
          dest: '<%= config.dist %>/chattGtfsCleanupGui.app'
        }]
      },
      zipToApp: {
        files: [{
  ...
  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/chattGtfsCleanupGui.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/chattGtfsCleanupGui.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/chattGtfsCleanupGui.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/chattGtfsCleanupGui.app/Contents/MacOS/node-webkit', '555');
  });
  ...
  grunt.registerTask('createWindowsApp', 'Create windows distribution.', function () {
    ...
    concat([
      'tmp/nw.exe',
      'tmp/app.nw'
    ], 'tmp/chattGtfsCleanupGui.exe', function () {
      var fs = require('fs');
  ...
  grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
    var done = this.async();
    var createDmgCommand = 'resources/mac/package.sh "chattGtfsCleanupGui"';
